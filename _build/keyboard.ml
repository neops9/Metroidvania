...
let keyboard_actions s = 
  ...
  begin
    ...
    if (!player).current_animation.name = "hurt" && (!player).current_animation.loop > 0 then begin
      player := { !player with in_air = true; vy = -12.1 }; end
    else
      begin player := { !player with current_animation = get_animation_from_list (!player).animations "jump"; in_air = true; vy = -12.1 }; end
  end;
  ...
  if (keystates.{ Tool.scancode "a" } != 0 && ((!player).reload_time <= 0)) then
    begin
      ...
      begin
        ...
        if (!player).current_animation.name = "hurt" && (!player).current_animation.loop > 0 then
          begin player := { !player with reload_time = 20; projectiles = projectile::((!player).projectiles) }; end
        else
          begin player := { !player with current_animation = get_animation_from_list (!player).animations "throw"; reload_time = 20; projectiles = projectile::((!player).projectiles) }; end
      end
      else
      begin
