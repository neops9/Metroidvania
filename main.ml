...
let print_image r i =  match Sdl.load_bmp i with
  ...
  | Ok s -> match Sdl.render_copy r (create_texture_from_surface r s)  with
            | Error (`Msg e) ->  Sdl.log "Cant fill image  error: %s" e; exit 1 
	    | Ok () -> Sdl.render_present r
;;

let get_window_surface w  =  match Sdl.get_window_surface w with
  | Error (`Msg e) -> Sdl.log "Cant get Surface  error: %s" e; exit 1
  | Ok s -> s
;;



let rec wait w = let event = Sdl.Event.create ()  in match Sdl.wait_event (Some(event)) with
...
| Ok () ->
    ...
    | Ok w ->
       ...
       | Error (`Msg e) ->  Sdl.log "Cant create renderer  error: %s" e; exit 1
