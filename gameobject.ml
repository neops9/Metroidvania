...
let create name x y vx vy current_animation animations collision flip life life_time sounds is_projectile = { name;
                                                                                                              x;
                                                                                                              y;
                                                                                                              vx;
                                                                                                              vy;
  ...
  then o
    ...
    begin
      ...
      then
	    ...
	    if collision_rec (gameobject_to_rect o_y) l
            then
              { o with projectiles = List.map (move l) (o.projectiles) }
	    else
              { o_y with projectiles = List.map (move l) (o_y.projectiles) }
          end
      else
        let o_x_y = { o_x with y = o_x.y + int_of_float(o_x.vy) } in
        if collision_rec (gameobject_to_rect o_x_y) l
        then 
          begin
            if o.is_projectile
            then
              ...
              { o_x with projectiles = List.map (move l) (o_x.projectiles) }
          end
        else
          { o_x_y with projectiles = List.map (move l) (o_x_y.projectiles) }
    end
;;

let rec update_projectiles o = { o with projectiles = List.fold_left (fun acc x -> if x.life_time <= 0
                                                                                   then acc
                                                                                   else ({ x with vy = x.vy +. 0.5;
                                                                                                  current_animation = Animation.update x.current_animation;
                                                                                                  life_time = x.life_time - 10 })::acc) [] o.projectiles }
;;
...
let update o = 
  ...
  then
    { o with vy = o.vy +. 0.5;
             current_animation = Animation.update o.current_animation;
             life_time = o.life_time - 10 }
  else 
    { o with current_animation = Animation.update o.current_animation;
             life_time = o.life_time - 10 }
;;
...
let destroy o = Animation.destroy o.current_animation;
                List.iter (Animation.destroy) o.animations
;;
