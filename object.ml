...
type objet = { x : int; y : int; vy : float; vx : int; dx :int; dy : int; frame : int; texture : Sdl.texture; life_time : int; in_air : bool; flip : bool; collision : bool; character : bool; bullet_time : int } ;;
let create x y texture vy vx dx dy frame life_time in_air collision character = { x; y; vy; vx; dx; dy; frame; texture; life_time; in_air; flip = false; collision; character; bullet_time = 0 } ;;
let get_x o = o.x;;
...
let update o r = { o with vy = o.vy +. 0.5; texture = (Tool.create_texture_from_image r ("images/char"^((string_of_int) (o.frame / 5))^".bmp")); bullet_time = o.bullet_time - 1 } ;; 
