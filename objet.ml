...
type objet = { x : int; y : int; vy : float; vx : int; dx :int; dy : int; frame : int; texture : Sdl.texture; life_time : int; in_air : bool; flip : bool; collision : bool; character : bool; bullet_time : int; life : int; sounds : son list; is_bullet : bool; invulnerable : int } ;;
let create x y texture vy vx dx dy frame life_time in_air collision character life sounds is_bullet = { x; y; vy; vx; dx; dy; frame; texture; life_time; in_air; flip = false; collision; character; bullet_time = 0; life; sounds; is_bullet; invulnerable = 0 } ;;
let get_x o = o.x;;
